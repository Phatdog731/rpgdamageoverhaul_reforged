...
public enum ResistanceComponentProvider implements IEntityComponentProvider, IServerDataProvider<EntityAccessor> {
    ...
    public ResourceLocation getUid() {
        return ResourceLocation.fromNamespaceAndPath(RPGDamageOverhaul.MODID, "resistances");
    }
    ...
    public void appendServerData(CompoundTag nbt, EntityAccessor entityAccessor) {
        ...
        {
            var attrHolder = BuiltInRegistries.ATTRIBUTE.wrapAsHolder(dc.resistanceAttribute);
            if (le.getAttribute(attrHolder) == null)
                continue;
            var val = le.getAttributeValue(attrHolder);
            if (val == 0)
                continue;
